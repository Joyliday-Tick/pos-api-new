...
func CreateAdjustPoint(c *gin.Context) {
	...
	// ยอด E-Stamp อยู่ที่คอลัมน์ ecoin — คอลัมน์ estamp ไม่ได้ใช้งาน
	// (เจ้าของระบบยืนยัน 2026-09-19) การตรวจเพดานกับ Ecoin จึงถูกต้องอยู่แล้ว
	// ส่วนข้อความ error ที่เคยต้องอธิบายว่า "หักได้ไม่เกินยอด eCoin" ไม่ต้องแล้ว
	// เพราะ eCoin กับ E-Stamp เป็นยอดเดียวกัน ไม่ใช่สองยอดที่ต้องเทียบกัน
	if req.AdjEstamp < 0 {
		...
		if findMember.Ecoin < deductAmount {
			utils.Error(c, http.StatusBadRequest, fmt.Sprintf(
				"E-Stamp ไม่เพียงพอ ในระบบมี %v (ขอหัก %v)",
