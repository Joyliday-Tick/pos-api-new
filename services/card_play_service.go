...
func DeductCardPlay(input models.CardPlayDeductDto, userId int) ([]Deposit, error) {
	...
	if err == nil {
		...
		if playTime > 0 {
			...
			//
			// เจ้าของระบบตัดสินเมื่อ 2026-09-18 ว่า **ยังไม่ต้องบล็อก**
			// เพดานที่ใช้จริงคือยอดในบัตร ไม่ใช่จำนวนครั้ง
			// นี่จึงเป็นพฤติกรรมที่ต้องการ ไม่ใช่งานค้าง — อย่าเปลี่ยนเป็น
			// return error โดยไม่ถามเจ้าของระบบก่อน
			//
			// คง log ไว้เพื่อให้รู้ว่าเกิดบ่อยแค่ไหน ถ้าวันหนึ่งอยากบล็อก
			// ข้อมูลจาก log จะบอกได้ว่ากระทบเท่าไร
