...
**ก. `CheckCardInfo` รายงานยอดเป็น 0 ทั้งที่บัตรมีเงิน — กระทบลูกค้าโดยตรง**

`LEFT JOIN LATERAL` ใน `CheckCardInfo` รวมเฉพาะ deposit ที่ `member_tel` ตรงกับ
`card_entity.member_tel` ถ้าไม่ตรง ยอดที่แสดงเป็น 0

ตรวจ UAT 18 ก.ย.: **บัตร 2 ใบ รวม 361 coin** ที่มียอดดิบแต่หน้าจอแสดง 0
ลูกค้าถือเงินอยู่แต่มองไม่เห็นและใช้ไม่ได้ และก่อนมี guard ข้างบน บัตรพวกนี้
จะถูกล้างทันทีถ้ามีใคร void บิลของมัน
