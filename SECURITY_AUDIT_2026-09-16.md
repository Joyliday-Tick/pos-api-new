...
อัปเดต 24 ก.ย.: **C10 แก้แล้ว** (`a41bd06` — `float32` → `float64`) · **C11 เจ้าของระบบ
...
### ~~10. `discount_cash` ใช้ `float32`~~ — แก้แล้ว 24 ก.ย. `a41bd06`
...
> **เหตุผลที่ขีดฆ่าผิด** (ตรวจ 24 ก.ย.) — คอลัมน์ `discount_cash` และ
...
### 11. ไม่มี idempotency ที่ไหนเลย — **เจ้าของระบบตัดสินแล้ว 24 ก.ย. ว่าไม่ทำ**
...
**สิ่งที่วัดได้จริงเมื่อ 24 ก.ย. ทำให้ขอบเขตแคบกว่าที่เขียนไว้เดิม:**
...
**เจ้าของระบบตัดสินเมื่อ 24 ก.ย. ว่าไม่ต้องทำอะไรกับฝั่ง KIOSK** — จึงไม่ใส่ guard นี้ **ถือเป็นข้อยุติ ไม่ใช่งานค้าง** อย่าย้อนมาเพิ่มการเช็ค `bill_no` ซ้ำภายหลังโดยอ้างว่า "เพื่อความสม่ำเสมอกับ void/refund/claim"
