...
## สถานะการแก้ (อัปเดต 2026-09-16)

repo นี้ถูก `git init` ในวันเดียวกัน `fcd160a` คือ baseline ก่อนแก้ — `git diff fcd160a` เพื่อดูของที่เปลี่ยนทั้งหมด

**แก้แล้ว** (build + `go vet` ผ่านทั้งคู่ใน `golang:1.24`):

| ข้อ | commit |
|---|---|
| B2 — API คืนรหัสผ่าน plaintext | `9ad4ad4` |
| B4 — log รหัสผ่าน/token/JWT secret | `73b3034` |
| B5 — Basic auth panic จาก header ไม่มี `:` | `73b3034` |
| A2 — `.env` ติดไปกับ docker image | `fd8185e` |
| C1 — หักยอดไม่สำเร็จแต่ตอบสำเร็จ (เล่นฟรี) | `455d06e` |
| C9 — `if err != nil` กลับด้าน | `fbcd244` |
| C5 — เลขบิลซ้ำทุกวัน | `b90e38e` ⚠️ เปลี่ยนรูปแบบเลขบิล ต้องยืนยันกับฝ่ายบัญชีก่อน deploy |
| A1 — connection pool | `a3c1f2b` ตั้ง pool ครบทั้ง 4 DB จากโควตาที่วัดจริง (POS = 50 ไม่ใช่ 100) |
| A3 — JWT secret เป็นค่าว่าง | `InitJwtSecret()` อ่านหลัง `godotenv.Load()` และ `log.Fatal` ถ้าว่างหรือสั้นกว่า 32 |
| C3 + C6 + C8 — double-spend, จำนวนติดลบ, TOCTOU | `aff52a5` |
| C4 — เติมเงินก่อนสร้างบิล + panic จาก input ปกติ | `5c4ab1d` (กัน panic แล้ว แต่ยังไม่ atomic) |
| C2 + C7 — void ซ้ำ, รันขนานแบบไม่มีใครหยุดใคร | `7bd5846` |
| B3 — ไม่มีการตรวจสิทธิ์ฝั่งเซิร์ฟเวอร์ | `280dbc6` ⚠️ อ่านข้อจำกัดด้านล่าง |

**C3/C6/C8 แก้ที่จุดเดียว** — `UpdateCardDepositBalance` เพราะทุกเส้นทางที่หักยอด
(card play, void, claim-prize, refund) เรียกผ่านฟังก์ชันนี้ทั้งหมด
เงื่อนไขย้ายเข้า `WHERE ... AND COALESCE(balance_coin,0) >= ?` แล้วเช็ค `RowsAffected`
พร้อมกันยอดหักติดลบ และแปลง `BalanceDiscountCash` ที่เป็น `nil` ให้เป็น 0 แทน NULL

### ยืนยันกับข้อมูล UAT จริง (อ่านอย่างเดียว 2026-09-17)

ข้อเหล่านี้ **ไม่ใช่ความเสี่ยงเชิงทฤษฎี** จาก `card_deposit` 2678 แถว:

| อาการ | จำนวน | ตรงกับข้อ |
|---|---|---|
| `balance_discount_cash IS NULL` | 9 | บั๊ก nil→NULL (ไม่อยู่ในรายงานฉบับแรก) |
| `balance_coin < 0` | 5 | C2 / C3 |
| `balance_bonus < 0` | 1 | C2 / C3 |

แถวที่ติดลบกระจุกอยู่ที่บัตร `A4BD285F` (−1, −2, −3, −18 อัปเดตวันเดียวกัน 2026-02-04)
และ `AAF5CF2F` (−30 / −20) รูปแบบตรงกับ void ซ้ำ/double-spend ที่รายงานไว้

**ยังไม่ได้แตะข้อมูล** — guard ใหม่จะทำให้ 5 แถวนี้หักไม่ผ่านและคืน error
ซึ่งตั้งใจให้ดังไว้ก่อน การซ่อมข้อมูลเป็นการตัดสินใจแยกต่างหาก

**ยังไม่แก้** — เหลือ **B1 เท่านั้น** (bcrypt ซึ่งผู้ใช้เลื่อนไว้โดยตั้งใจ)

อัปเดต 19 ก.ย.: **C10 แก้แล้ว** (`a41bd06` — `float32` → `float64`) · **C11 เจ้าของระบบ
ตัดสินแล้วว่าไม่ทำ** ทั้งสองข้อเป็นข้อยุติ ไม่ใช่งานค้าง ดูรายละเอียดในข้อของมัน
(B1 bcrypt = ผู้ใช้ตัดสินใจเลื่อนไว้ก่อน จนกว่าจะย้ายทั้งระบบ ดู [[two-systems-one-database]])

### ที่ยังค้างอยู่หลังแก้ C2/C4/C7

สามข้อนี้ "กันพังแล้ว" แต่ยัง **ไม่ atomic จริง** เพราะต้องรื้อโครงมากกว่านี้:

1. **C4 — topup ยังไม่อยู่ใน transaction เดียว** `card_deposit` ถูกสร้างใน goroutine
   ขนานก่อน `pos_transaction` เสมอ ถ้าพังหลังจากนั้นจะเหลือยอดลอย
   `5c4ab1d` ปิดสาเหตุที่เกิดบ่อยที่สุด (panic จาก pointer/UUID) และเพิ่ม log
   `[ORPHAN DEPOSIT]` ให้ตามเก็บได้ แต่ยังไม่ได้ทำให้ rollback ได้
   จะแก้จริงต้องรื้อ goroutine เป็น sequential ใน `DB_POS.Transaction()`

2. **`pos_void.bill_no` ยังไม่ unique** ดัชนีปัจจุบันคือ `idx_pos_void_bill` แบบธรรมดา
   การจองบิลใน `7bd5846` จึงยังมีช่องแข่งกันถ้ายิงพร้อมกันจริงระดับมิลลิวินาที
   ตรวจ UAT แล้ว **ไม่มี `bill_no` ซ้ำเลย** เพิ่ม unique index ได้สะอาด:

   ```sql
   CREATE UNIQUE INDEX CONCURRENTLY idx_pos_void_bill_unique
     ON pos_void (LOWER(bill_no));
   ```

   `LOWER()` เพื่อให้ตรงกับที่ `FindExistVoidByBillNo` ค้นหา
   **เป็น schema change บนฐานข้อมูลที่ใช้ร่วมกับระบบเดิม ต้องให้เจ้าของตัดสินใจ**
   ดู [[two-systems-one-database]]

3. **B3 จำกัดหน้าจอ POS ไม่ได้** `pos-fe-new` ไม่ได้ส่งตัวตนของพนักงานที่ล็อกอิน
   ขึ้นมา แต่เรียก API ด้วย service account ตัวเดียวชื่อ `pos_frontend`
   (users.id 751) ซึ่งมี `user_role_id = 1` = Administrator
   ทุกคำขอจากหน้าจอ POS จึงมาถึง backend ในฐานะแอดมิน

   `RequireRole` จึง **ไม่ได้จำกัดสิ่งที่หน้าจอ POS ทำได้เลย** สิ่งที่ปิดได้คือ
   ช่องที่รายงานไว้จริง: พนักงานเอา username/password ของตัวเอง (ชุดเดียวกับที่
   หน้า login ของ POS ใช้ตรวจกับ `pos-api`) ไปยิง API ตรงเพื่อข้ามการขออนุมัติ
   จากหัวหน้าที่กั้นไว้แค่ใน UI — คำขอแบบนั้นมี `roleId` ของพนักงานจริงและถูกปฏิเสธ

   ถ้าจะให้การตรวจสิทธิ์มีผลกับเส้นทางปกติด้วย ต้องเลิกใช้ service account ร่วมกัน
   แล้วส่งตัวตนพนักงานขึ้นมา ซึ่งแตะ `src/lib/getHeaderToken.js` และชั้น session

4. **CRM sync ยัง rollback ไม่ได้** อยู่ขั้นสุดท้ายแล้ว ถ้าพังจะได้สภาพ
   "ยกเลิกในระบบ POS ครบ แต่คะแนนสมาชิกยังไม่ถูกหัก" ซึ่ง log ไว้ให้ตามแก้ด้วยมือ
   ระบบภายนอกจึงทำได้แค่นี้ นอกจากจะทำ outbox/retry queue

หมายเหตุ B2: รายงานฉบับแรกระบุให้แก้ `models/user.go` บรรทัด 7, 18, 74 ด้วย — **ผิด** บรรทัดเหล่านั้นเป็น `LoginRequest` และ DTO ขาเข้า ถ้าใส่ `json:"-"` จะ login และสร้างผู้ใช้ไม่ได้ แก้จริงเฉพาะ `UsersData.Password` กับ `Users.Password` ซึ่งเป็นขาออกอย่างเดียว
...
### ~~10. `discount_cash` ใช้ `float32`~~ — แก้แล้ว 19 ก.ย. `a41bd06`
...
> **เหตุผลที่ขีดฆ่าผิด** (ตรวจ 19 ก.ย.) — คอลัมน์ `discount_cash` และ
> `balance_discount_cash` เป็น **`numeric(8,2)`** ไม่ใช่ float ฐานข้อมูลปัดเป็น
> ทศนิยมสองตำแหน่งทุกครั้งที่เขียน เศษ `-0.0000019` จึงเกิดในฐานข้อมูลไม่ได้
> และข้อมูลจริง 2,704 แถวไม่มีแถวไหนติดลบหรือมีเศษต่ำกว่า 0.01 เลย

**ของที่เป็นปัญหาจริงคือฝั่ง Go** `float32` เก็บเลขนัยสำคัญได้ราว 7 หลัก ขณะที่คอลัมน์รับได้ถึง `999999.99` ซึ่งเป็น 8 หลัก ค่าที่ใกล้เพดานจึงเพี้ยนตั้งแต่ตอนแปลงชนิด (`999999.99` → `1000000.00`) ไม่ใช่ตอนลบซ้ำ ๆ

**แก้แล้ว** เปลี่ยนเป็น `float64` ทั้ง 20 จุด (`float64` มีราว 15-16 หลัก ครอบคลุม `numeric(8,2)` เต็ม) ไม่แตะ schema จึงไม่กระทบระบบเก่าที่ใช้ฐานร่วมกัน · `go build` + `go vet` ผ่าน

### 11. ไม่มี idempotency ที่ไหนเลย — **เจ้าของระบบตัดสินแล้ว 19 ก.ย. ว่าไม่ทำ**

ค้นทั้ง codebase: ไม่มี idempotency key ไม่มี request ID ไม่มี `uniqueIndex` tag ในโมเดลใดเลย

`TopupCardPOS` รับ `bill_no` มาจาก client ได้ (`models/card_entity.go:57`) และใช้ค่านั้นตรง ๆ **โดยไม่เช็คว่าซ้ำหรือไม่** → ส่งเติม 1000 สำเร็จแต่ response หาย → ส่งซ้ำ → **บัตรได้ 2000 จากการจ่ายเงินครั้งเดียว**

**สิ่งที่วัดได้จริงเมื่อ 19 ก.ย. ทำให้ขอบเขตแคบกว่าที่เขียนไว้เดิม:**

- **หน้าจอ POS ของเราไม่ส่ง `bill_no` เลย** — `topup/page.jsx:930` อ่านค่าที่ตอบกลับมาอย่างเดียว เซิร์ฟเวอร์เป็นคนสร้างเลขเองผ่าน `GenerateBillNo` ช่องนี้จึงเปิดเฉพาะกับ client ที่ส่งเลขมาเอง ซึ่งในข้อมูลคือฝั่ง KIOSK (เลขขึ้นต้น `KIOSK...`) ที่เราไม่เห็นโค้ด
- **ร่องรอยการกดซ้ำในข้อมูลบางมาก** — จับคู่ `card_deposit` ที่บัตรเดียวกัน ยอดเดียวกัน คนละบิล ห่างกันไม่เกิน 60 วินาที ได้ 104 คู่ แต่เหลือเพียง **6 คู่ที่ห่างกันไม่ถึง 2 วินาที** และ 4 ใน 6 คู่นั้นคือ **การทดสอบของเราเองเมื่อ 18 ก.ย.** (บัตร `TEST024` prefix `UJN`) เหลือของจริง 2 คู่จาก 2,704 แถว หนึ่งในนั้นเป็นบัตร placeholder `Card ID`
- ส่วนอีก 80 คู่ที่ห่างกันเกิน 10 วินาที มีแนวโน้มเป็นการซื้อซ้ำจริงมากกว่าการกดซ้ำ

**ทางแก้ที่ถูกที่สุดและไม่ต้องแตะ schema:** `bill_no` ที่ client ส่งมาทำหน้าที่เป็น idempotency key ได้อยู่แล้ว — แค่ปฏิเสธ (409) ถ้าเลขนั้นมีอยู่ใน `pos_transaction` แล้ว ไม่ต้องมีตารางใหม่ ไม่ต้องมี header ใหม่ และเข้ากับแนวที่ void/refund/claim ทำไว้แล้ว

**เจ้าของระบบตัดสินเมื่อ 19 ก.ย. ว่าไม่ต้องทำอะไรกับฝั่ง KIOSK** — จึงไม่ใส่ guard นี้ **ถือเป็นข้อยุติ ไม่ใช่งานค้าง** อย่าย้อนมาเพิ่มการเช็ค `bill_no` ซ้ำภายหลังโดยอ้างว่า "เพื่อความสม่ำเสมอกับ void/refund/claim"

เหตุผลประกอบที่ทำให้การตัดสินนี้สมเหตุสมผล: เส้นทางที่เปิดอยู่ไม่ใช่เส้นทางของหน้าจอ POS (ซึ่งไม่ส่ง `bill_no` เลย) แต่เป็นของ client ที่เราไม่ได้ดูแลและมองไม่เห็นพฤติกรรม การใส่ guard ฝั่งเราจึงเป็นการเปลี่ยนสัญญากับระบบที่ยังทำงานอยู่ โดยไม่มีหลักฐานว่ามันกำลังเสียหาย — ข้อมูลบอกว่าปกติ 1 บิล = 1 แถว (1,598 จาก 1,621 เลขบิล) แต่มี 20 เลขที่มีหลายแถว ถ้า KIOSK ตั้งใจยิงหลายครั้งด้วยเลขเดียวกัน guard นี้จะไปตัดของที่ใช้งานอยู่จริง

ส่วน idempotency แบบทั่วไป (ตารางเก็บ key + middleware) ต้องเพิ่มตารางในฐานที่ใช้ร่วมกับระบบเก่า และต้องให้ฝั่งหน้าเว็บส่ง key มาด้วย เป็นงานข้าม repo ที่ควรตัดสินใจแยก ไม่ใช่แถมมากับการแก้บั๊ก

**สิ่งที่คุ้มครองเส้นทางเงินวันนี้จึงเป็นความถูกต้องของตรรกะ ไม่ใช่ idempotency** — void / refund / claim ตรวจสถานะแล้วตอบ 409 เมื่อกดซ้ำ (ทดสอบแล้ว 18 ก.ย.) และการหักยอดย้ายเงื่อนไขเข้า `WHERE` แล้วเช็ค `RowsAffected` จึงหักซ้ำไม่ได้
